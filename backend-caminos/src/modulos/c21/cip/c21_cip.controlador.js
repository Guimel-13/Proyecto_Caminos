const C21CipModelo = require('./c21_cip.modelo');
const fs = require('fs');
const path = require('path');

const listarRegistros = async (req, res) => {
    try {
        const registros = await C21CipModelo.obtenerTodosLosRegistros(req.query.gestion);
        res.json(registros);
...
const crearRegistro = async (req, res) => {
    try {
        ...
        const nuevoRegistro = await C21CipModelo.guardarNuevoRegistro(datos);

        if (datos.archivo_temporal) {
            const tempPath = path.join(__dirname, '../../../../../temp_scans', datos.archivo_temporal);
            if (fs.existsSync(tempPath)) {
                // Carpeta única para C21 CIP
                const carpetaFinal = path.join(__dirname, '../../../../public/archivos', `${datos.gestion}_C21_CIP_PDF`);
                if (!fs.existsSync(carpetaFinal)) fs.mkdirSync(carpetaFinal, { recursive: true });

                const nombreFinal = `C21_CIP_${nuevoRegistro.id}.pdf`;
                fs.renameSync(tempPath, path.join(carpetaFinal, nombreFinal));

                const rutaWeb = `archivos/${datos.gestion}_C21_CIP_PDF/${nombreFinal}`;
                await C21CipModelo.modificarRegistro(nuevoRegistro.id, { ...datos, ruta_archivo: rutaWeb });
...
const editarRegistro = async (req, res) => {
    try {
        const { id } = req.params;
        const datos = req.body;
        const registroAntiguo = await C21CipModelo.obtenerRegistroPorId(id);
        ...
        if (datos.archivo_temporal) {
            ...
            if (fs.existsSync(tempPath)) {
                ...
                const nombreFinal = `C21_CIP_${id}.pdf`;
                ...
                rutaFinalWeb = `archivos/${datos.gestion}_C21_CIP_PDF/${nombreFinal}`;
            }
        ...
        const registroActualizado = await C21CipModelo.modificarRegistro(id, datos);
...
const borrarRegistro = async (req, res) => {
    try {
        const registroEliminado = await C21CipModelo.eliminarRegistro(req.params.id);
